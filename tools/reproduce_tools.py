...
import utils
from inrf.inrf import INRF2D
from inrf.inr_maps_2d import plot_graph
...
def main():
    ...
    for idx in range(len(files)):
        ...
        inrf = INRF2D(noise_dim=metadata['noise_dim'],
                    x_dim=args.x_dim,
                    y_dim=args.y_dim,
        ...
        if random == 'graph':
            ...
            activations = 'basic'
            graph_topology = 'basic'
        inrf.init_map_fn(activations=activations,
        ...
        if metadata['graph'] is not None:
            random = 'graph'
            num_nodes = inrf.generator.graph.number_of_nodes()
        ...
        if args.generate_zoom_set:
            ...
            for i, zoom_z in enumerate(zoom_set):
                zoom = (zoom_z, zoom_z)
                frame = inrf.sample(noise, args.x_dim, args.y_dim, 1, zoom=zoom, splits=splits)
                frame = frame.cpu().detach().numpy()[0]*255
                frames.append(frame)
        else:
            print ('[START] Generating frame')
            frame = inrf.sample_frame(noise, args.x_dim, args.y_dim, 1, splits=1)
            frame = frame.cpu().detach().numpy()[0]*255
            frames = [frame]

        if args.draw_graph:
            if hasattr(inrf.generator, "get_graph"):
                graph = inrf.generator.get_graph()
            else:
                graph = inrf.generator.graph

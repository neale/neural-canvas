...
import utils
from inrf.inr_maps_3d import MapRandomGraph3D, plot_graph, MapRandomAct3D, Map3D
...
class CPPN(object):
    ...
    def _coordinates(self, zoom, pan, k):
        xpan, ypan, zpan = pan
        ...
        x_mat = np.matmul(np.ones((y_dim, 1)), x_range.reshape((1, x_dim)))[k]
        y_mat = np.matmul(y_range.reshape((y_dim, 1)), np.ones((1, x_dim)))[k]
        
        x_mat3 = (np.tile(x_mat[:,  None], [1, self.z_dim])).astype(np.float32)
        y_mat3 = (np.tile(y_mat[:,  None], [1, self.z_dim])).astype(np.float32)
        z_mat3 = (np.rot90(x_mat3, 1)).astype(np.float32)
    ...
    def sample_frame(self, noise, zoom=(.5,.5,.5), pan=(2,2,2), splits=1):
        for k in tqdm.tqdm(range(0, self.z_dim)):
            x_vec, y_vec, z_vec, r_vec, n_pts = self._coordinates(zoom, pan, k=k)
            ...
            noise_scale_i = noise_one_vec_i.view(-1, self.noise_dim)
            f = self.map_fn(x_vec, y_vec, z_vec, r_vec, noise_scale_i)#[split], y[split], z[split], r[split], noise_scale_i)
            #torch.save(f, f'f_temp_gen{split}.pt')
            fs = f.reshape(self.x_dim, self.y_dim, 3)
            torch.save(fs, f'f_temp_gen_{k}.pt')
        #gc.collect()
        x = np.ones((self.z_dim, self.x_dim, self.y_dim, 3), dtype='u1')
        #x = torch.ones(self.z_dim, self.x_dim, self.y_dim, 3)
        x[0] = (torch.load(f'f_temp_gen_{0}.pt').numpy() * 255.).astype('u1')
        #os.remove(f'f_temp_gen_{0}.pt')
        for n in tqdm.tqdm(range(1, self.z_dim)):
            x[n] = (torch.load(f'f_temp_gen_{n}.pt').numpy() * 255.).astype('u1')
            #x[n] = torch.load(f'f_temp_gen_{n}.pt')
        #    os.remove(f'f_temp_gen_{k}.pt')
...
def run_cppn(cppn, autosave=False, z=None, debug=False):
    ...
    if cppn.x_dim > 256 and cppn.x_dim <= 512:
        ...
        splits = 64 - (cppn.x_dim % 512)
    else:
        splits = 1
    
    cppn.logger.info(f'Calculated {splits} splits')
    batch_samples = []
    for i, z_i in enumerate(zs):
        ...
        utils.write_image(path=save_fn, img=frame, suffix='jpg')
...
def load_args():
    ...
    parser.add_argument('--n_samples', default=2, type=int, help='images to generate')
...
if __name__ == '__main__':
    args = load_args()
    noise = np.array([8, 16, 34, 32, 48, 64])
    scale = np.array([.1, .5, 1, 2, 4, 10, 32, 64])
    w = np.array([4, 16, 32, 64])
    c_dim = np.array([3])
    w_mean = np.array([-3, -2, -1, 0, 1, 2, 3])
    w_std = np.array([.85, 1.0, 1.15])
    graph_nodes = np.array([4, 6, 10, 20, 30])
